...
func HandleCreateFile(node *common.Node, req CreateFileRequest) (*CreateFileResponse, error) {
	...
	for _, replica := range replicas {
		if replica.Hostname == selfInfo.Hostname && replica.Port == selfInfo.Port {
			isSelfReplica = true
			log.Printf("ℹ️  CREATE: Coordinator node IS also a replica (will save locally)")
			break
		}
	}
	if !isSelfReplica {
		log.Printf("ℹ️  CREATE: Coordinator node is NOT a replica (will only forward requests)")
	}
	...
	for _, replica := range replicas {
		...
		if err != nil {
			log.Printf("⚠️  CREATE: Failed to replicate to %s:%d: %v", replica.Hostname, replica.Port, err)
			continue
		}

		if !resp.Success {
			log.Printf("⚠️  CREATE: Replication to %s:%d failed: %s", replica.Hostname, replica.Port, resp.Error)
			continue
		}
